...
USE `hospital`;
DROP procedure IF EXISTS `info_paciente`;

USE `hospital`;
DROP procedure IF EXISTS `hospital`.`info_paciente`;
;

DELIMITER $$
USE `hospital`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `info_paciente`(in edad int)
BEGIN
select * from paciente where paciente.edad = edad;
